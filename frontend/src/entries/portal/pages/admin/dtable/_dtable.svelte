...
<div class="h-full w-full overflow-auto">
  <div class="md:p-12 bg-indigo-100 flex flex-row flex-wrap">
    ...
    >
      <div class="text-2xl text-indigo-900">Data Table</div>
      ...
      <div class="flex-col flex py-3 relative">
        <label class="pb-2 text-gray-700 font-semibold">Main column</label>
        <input
          type="text"
          value={data["main_column"] || ""}
          on:change={set("main_column")}
          class="p-2 shadow rounded-lg bg-gray-100 outline-none focus:bg-gray-200"
        />
      </div>
      ...
      <div
        ...
        on:click={async () => {
          ...
          await dynapi.edit_dtable(source, group, data["slug"], _data);
          app.navigator.goto_admin_dgroup_page(source, group);
        }}
      >
