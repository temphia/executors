<script lang="ts">
  ...
  const load = async () => {
    const api = await app.apm.get_cabinet_api(source);
    const resp = await api.list_folder(folder);
    files = resp.data;
    files_loaded = true;
  ...
  const finalUpload = async (file, data) => {
    const api = await app.apm.get_cabinet_api(source);
    api.upload_file(folder, file, data);
    load();
    close();
  };

  const show_upload_panel = () => {
    open(PanelUploadFile, { uploadFile: finalUpload });
  };
          ...
          {#each files as file}
            <tr class="text-gray-700 hover:bg-gray-200">
              ...
              <td class="px-4 py-1 text-sm border">
                <button
                  on:click={() => {
                    app.navigator.goto_cabinet_file(source, folder, file.name);
                  }}
                ...
                <button
                  on:click={async () => {
                    const api = await app.apm.get_cabinet_api(source);
                    await api.delete_file(folder, file.name);
                    load();
                  }}
