<script lang="ts">
    const timelines = [
      {
        ...
        assoc_user: "user12",
        assoc_version: 0,
        mod_sig: "xyxyxyxyyxyxyxyxyxyxyxyxyxy",
      },
      {
        type: "update_record",
        fields: ["deadline", "target"],
  ...
  <div class="p-2">
    ...
    <div class="w-full p-3 mt-4 bg-white rounded shadow flex flex-shrink-0">
      ...
      <div class="pl-3 w-full">
        <div class="flex items-center justify-between w-full">
          <p tabindex="0" class="focus:outline-none text-sm leading-none">
            <span class="text-indigo-700">Sash</span> added you to the group:
    ...
    <div class="w-full p-3 mt-4 bg-white rounded flex">
      ...
      <div class="pl-3">
        <p tabindex="0" class="focus:outline-none text-sm leading-none">
          Shipmet delayed for order<span class="text-indigo-700"> #25551</span>
        </p>
        <p
          tabindex="0"
          class="focus:outline-none text-xs leading-3 pt-1 text-gray-500"
  ...
  <div class="p-2">
    {#each timelines as tl}
      <div
        class="transform border transition cursor-pointer hover:-translate-y-2 ml-5 relative flex items-center px-2 py-1 rounded mb-5 md:flex-row space-y-2 hover:border-purple-600"
      >
  
        <div
          class="w-5 h-5 bg-blue-600 absolute -left-5 transform -translate-x-2/4 rounded-full z-10 mt-2 md:mt-0"
        />
  
  
        <div class="w-5 h-1 bg-blue-300 absolute -left-5 z-0" />
        ...
        <div class="flex items-center space-x-1">
          ...
          <div>
            <span class="text-blue-300 text-sm p-1 rounded">
              #{tl.mod_sig}
            </span>
          </div>
          <div class="flex gap-x-1">
            {#each tl.fields as field}
              <span class="bg-gray-100 text-sm p-1 rounded">{field}</span>
            {/each}
          </div>
  
          <div>
            <span class="text-sm p-1 rounded text-white {classes[tl.type] || ''}">
              {tl.type}
            </span>
          </div>
        </div>
